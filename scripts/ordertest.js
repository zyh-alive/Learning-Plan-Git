...
async function processes(token) {
    try {
        ...
        return {token,success: res.ok,time: time};
    } catch (err) {
...
async function test() {
    ...
    for (let i = 0; i < total; i++) {
        ...
        let res_token = await fetch(BASE + '/auth/login', {
            ...
            body: JSON.stringify({ phone, password: '666666', role: 'user' })
        });
        const token = await res_token.json();
        promises.push(processes(token.token));//返回一个对象，包含请求的响应状态、请求时间、token和错误信息
